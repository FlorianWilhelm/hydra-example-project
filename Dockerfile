...
# and `pip install hydra-example-project` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "hydra-example-project", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n hydra-example-project -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "hydra-example-project", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "hydra-example-project", "YOUR_CONSOLE_SCRIPT"]
